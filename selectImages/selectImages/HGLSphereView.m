...
@interface HGLSphereView() <UIGestureRecognizerDelegate>

@end

@implementation HGLSphereView
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    HGLPoint normalDirection;
    CGPoint last;
...
- (void)setCloudTags:(NSArray *)array
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        HGLPoint point = HGLPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(HGLPoint)];
    ...
    normalDirection = HGLPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(HGLPoint)direction andAngle:(CGFloat)angle
{
    ...
    HGLPoint rPoint = HGLPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(HGLPoint)];
...
- (void)setTagOfPoint: (HGLPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        HGLPoint direction = HGLPointMake(last.y - current.y, current.x - last.x, 0);
